...
const onStart: Middleware<ContextModel> = async(ctx, next) => {
  ...
  await ctx.api.sendMessage(receiver.telegram.id, `
    ✅ <b>${ctx.$user.telegram.first_name} transferred you ${amount.toFixed(2)} coins</b>

    Current balance: ${(receiverBalance + amount).toFixed(2)} coins
  `.replace(/^ +/gm, ""), {
    parse_mode: "HTML"
  });
  ...
  return ctx.reply(`
    ✅ <b>You've transferred ${amount.toFixed(2)} coins.</b>
    
    Current balance: ${(balance - amount).toFixed(2)} coins
  `.replace(/^ +/gm, ""), {
    parse_mode: "HTML",
